...
// "time remaining" estimate that shrinks as the candidate moves through
// the wizard, rather than showing a single fixed figure throughout.
const STEP_TIME_ESTIMATES_MIN: Record<number, number> = {
  1: 4,
  2: 3,
  3: 5,
  4: 2,
...
function estimateMinutesRemaining(activeStep: number): number {
  let total = 0;
  for (let step = activeStep; step <= TOTAL_STEPS; step += 1) {
    total += STEP_TIME_ESTIMATES_MIN[step] ?? 0;
  }
  return total;
}
...
export function RegistrationHero({
  ...
  const progressPercent = isSubmitted ? 100 : Math.round((activeStep / TOTAL_STEPS) * 100);
  const minutesRemaining = estimateMinutesRemaining(activeStep);
  const avatarLocked = isSubmitted && !editingUnlockedByAdmin;
  ...
  return (
    <div className="flex flex-col w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
              ...
              {isSubmitted && (
                <div className="flex items-center gap-2 w-fit text-label-sm text-candidate-secondary-fixed bg-white/10 rounded-lg px-3 py-2">
                  <span className="material-symbols-outlined text-[16px]" aria-hidden="true">
                    {editingUnlockedByAdmin ? "lock_open" : "lock"}
                  </span>
                  {editingUnlockedByAdmin
                    ? "An admin has temporarily unlocked your profile for editing."
                    : "Your profile has been submitted and is locked. Contact us if something needs to change."}
                </div>
              )}
            </div>
            <div className="lg:col-span-4 bg-candidate-navy-surface/90 rounded-xl p-4 shadow-lg space-y-3">
              ...
              <div className="flex items-center justify-between text-label-sm text-candidate-secondary-fixed">
                {isSubmitted ? (
                  <span className="flex items-center gap-1 text-[#A7F3D0] font-semibold">
                    <span className="material-symbols-outlined text-[16px]" aria-hidden="true">
                      check_circle
                    </span>
                    Completed
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    ~{minutesRemaining} min{minutesRemaining === 1 ? "" : "s"} remaining
                  </span>
                )}
      ...
      <section className="w-full bg-surface-container-lowest shadow-sm sticky top-20 z-30">

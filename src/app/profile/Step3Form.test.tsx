...
describe("Step3Form", () => {
  it("shows no document-type-specific fields until a pill is selected", () => {
    render(<Step3Form initialValues={null} onSubmitted={vi.fn()} onAvatarMissing={vi.fn()} />);
  ...
  it("Save & Submit runs client-side validation before any network call", async () => {
    const user = userEvent.setup();
    render(<Step3Form initialValues={null} onSubmitted={vi.fn()} onAvatarMissing={vi.fn()} />);
  ...
  it("surfaces the avatar-missing error inline and calls onAvatarMissing when the server blocks submission", async () => {
  ...
  it("calls onSubmitted on a fully successful submit", async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
